...
# Bevorzugt Service-Role-Key (läuft nur serverseitig), fällt auf altes SUPABASE_KEY zurück
...
if LICENSE_REQUIRED:
    if not SUPABASE_AVAILABLE:
        st.error("🔒 Lizenzprüfung ist aktiviert, aber das Supabase-Python-Paket fehlt. Installiere `supabase-py` oder setze LICENSE_REQUIRED=false für Tests.")
        st.stop()
    if not SUPABASE_URL or not SUPABASE_KEY:
        st.error("🔒 Lizenzprüfung ist aktiviert, aber SUPABASE_URL oder SUPABASE_SERVICE_ROLE_KEY fehlen. Bitte .env prüfen.")
        st.stop()
